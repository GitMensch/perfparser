...
{
    struct DemangleLib {
        DemangleLib(const QString& name, const char* function, const QByteArray& prefix)
            : name(name)
            , prefix(prefix)
        {
    ...
    const auto demangleLibs = {
        DemangleLib(QStringLiteral("rustc_demangle"), "rustc_demangle", QByteArrayLiteral("_R")),
        DemangleLib(QStringLiteral("d_demangle"), "demangle_symbol", QByteArrayLiteral("_D")),
    };
    for (const auto& lib : demangleLibs) {
        loadDemangleLib(lib.name, lib.function, lib.prefix);
    }
}
